...
// #ifdef GGML_USE_HIPBLAS [jart]
    if (cparams.flash_attn && llamafile_has_amd_gpu()) {
        LLAMA_LOG_WARN("%s: flash_attn is not yet compatible with AMD GPUs builds - forcing off\n", __func__);
        cparams.flash_attn = false;
    }
// #endif
