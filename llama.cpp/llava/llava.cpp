    ...
    */
    ...
    if (newline_tmp->backend != GGML_BACKEND_TYPE_CPU) {
        if (newline_tmp->buffer == NULL) {
            fprintf(stderr, "newline_tmp tensor buffer is NULL\n");
        }
        ggml_backend_tensor_get(newline_tmp, model.newline->data, 0, ggml_nbytes(newline_tmp));
    } else {
        model.newline->data = newline_tmp->data;
        if (model.newline->data == NULL) {
            fprintf(stderr, "newline_tmp tensor data is NULL\n");
        }
    }
...
static bool encode_image_with_clip(clip_ctx * ctx_clip, int n_threads, const clip_image_u8 * img, float * image_embd, int * n_img_pos) {
    ...
    if (strcmp(mm_patch_merge_type, "spatial_unpad") != 0) {
        ...
        *n_img_pos = clip_n_patches(ctx_clip);
        ...
        tinylogf("%s: %d segments encoded in %8.2f ms\n", __func__, (int)img_res_v.size, (t_img_enc_batch_us - t_img_enc_start_us) / 1000.0);
    ...
    tinylogf("%s: image embedding created: %d tokens\n", __func__, *n_img_pos);
...
bool llava_validate_embed_size(const llama_context * ctx_llama, const clip_ctx * ctx_clip) {
    ...
    if (n_image_embd != n_llama_embd) {
        fprintf(stderr, "%s: embedding dim of the multimodal projector (%d) is not equal to that of LLaMA (%d). Make sure that you use the correct mmproj file.\n", __func__, n_image_embd, n_llama_embd);
        return false;
    }
    return true;
}
