...
void HighlightForth::feed(std::string *r, std::string_view input) {
    int c;
            ...
            if (!isspace(c)) {
                ...
                if (is_label_) {
                    ...
                    *r += word_;
                    t_ = SYNTAX;
                    closer_ = '\n';
                } else if (word_ == "(" || // inline comment, e.g. ( arg1 arg2 -- res1 )
                           word_ == ".(") { // printed comment, e.g. .( compiling... )
                    *r += HI_COMMENT;
                    *r += word_;
                    t_ = SYNTAX;
                    closer_ = ')';
...
void HighlightForth::flush(std::string *r) {
    switch (t_) {
    case NORMAL:
        if (is_label_) {
            *r += word_;
