...
std::string remove_ansi_sgr_codes(const std::string &input) {
    std::string result;
...
bool is_color_reset(const std::string &input) {
    int t = 0;
    int number = 0;
        ...
        for (int i = 0; i < ITERATIONS; ++i) {
            ...
            if (!is_color_reset(colorized)) {
                fprintf(stderr, "%s highlight failed to reset color: %`'s -> %`'s\n", kLanguages[l],
                        sauce.c_str(), colorized.c_str());
                exit(1);
            }
            std::string plain = remove_ansi_sgr_codes(colorized);
            if (sauce != plain) {
                fprintf(stderr, "%s highlight failed to preserve code: %`'s -> %`'s -> %`'s\n",
                        kLanguages[l], sauce.c_str(), colorized.c_str(), plain.c_str());
                exit(1);
            }
        }
        delete h;
