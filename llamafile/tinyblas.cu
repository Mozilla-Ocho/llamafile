...
template <typename SRC, typename DST> __device__ __forceinline__ DST typechange(SRC x) {
    static_assert(std::is_same<DST, SRC>::value, "write a specialization");
    return x;
}

template <> __device__ __forceinline__ float typechange(half x) {
    return __half2float(x);
}

template <> __device__ __forceinline__ half typechange(float x) {
...
static __device__ void matmul_block2d(tinyblasOperation_t transa, //
    ...
    for (blob = 0; blob < k; blob += BK) {
        ...
        for (l = 0; l < BK; ++l) {
            for (j = 0; j < TM; ++j)
                At[j] = As[(l * BM) + ii0 * TM + j];
            for (h = 0; h < TN; ++h)
                Bt[h] = Bs[(l * BN) + ii1 * TN + h];
            for (j = 0; j < TM; ++j)
                for (h = 0; h < TN; ++h)
                    Cs[j * TN + h] += At[j] * Bt[h];
        }
    ...
    for (j = 0; j < TM && x + j < m; ++j)
        for (l = 0; l < TN && y + l < n; ++l) {
            C[(x + j) + (y + l) * ldc] = alpha * typechange<SRC, DST>(Cs[j * TN + l]);
            // + beta * (*(C + (x + j) + (y + l) * ldc));
        }
    __syncthreads();
}
...
static __global__ void tinyblasS_entry(tinyblasOperation_t transa, //
    ...
    for (x = blockIdx.x * BM; x < m; x += jump1)
        for (y = blockIdx.y * BN; y < n; y += jump2)
            matmul_block2d<BM, BN, BK, TM, TN>(transa, transb, m, n, k, x, y, alpha, A, lda, B, ldb,
                                               beta, C, ldc);
}
...
static void tinyblasS_wrapper(tinyblasHandle_t stream, //
    ...
    tinyblasS_entry<BM, BN, BK, TM, TN>
        <<<maxblocks, maxthreads, (sizeof(float) * (BM * BK + BK * BN)), stream>>>(
...
tinyblasStatus_t tinyblasSgemm(tinyblasHandle_t stream, //
                               ...
                               const float *beta, //
                               float *C, int ldc) {
    tinyblasS_wrapper<32, 8, 128, 1, 2>(stream, transa, transb, m, n, k, *alpha, A, lda, B, ldb,
                                        *beta, C, ldc);
    return TINYBLAS_STATUS_SUCCESS;
}

static bool check_args(cudaDataType_t Atype, cudaDataType_t Btype, cudaDataType_t Ctype,
                       tinyblasComputeType_t computeType) {
    return Atype == CUDA_R_16F && //
           Btype == CUDA_R_16F && //
           (Ctype == CUDA_R_16F || //
            Ctype == CUDA_R_32F) && //
...
static __global__ void tinyblasGE_entry(tinyblasOperation_t transa, //
    ...
    for (x = blockIdx.x * BM; x < m; x += jump1)
        for (y = blockIdx.y * BN; y < n; y += jump2)
            matmul_block2d<BM, BN, BK, TM, TN, SRC, DST>(transa, transb, m, n, k, x, y, alpha, A,
...
static void tinyblasGE_wrapper(tinyblasHandle_t stream, //
    ...
    if (Ctype == CUDA_R_16F)
        tinyblasGE_entry<BM, BN, BK, TM, TN, SRC, half>
            <<<maxblocks, maxthreads, (sizeof(SRC) * (BM * BK + BK * BN)), stream>>>(
                transa, transb, m, n, k, __float2half(alpha), A, lda, B, ldb, __float2half(beta),
                (half *)C, ldc);
    else
        tinyblasGE_entry<BM, BN, BK, TM, TN, SRC, float>
            <<<maxblocks, maxthreads, (sizeof(SRC) * (BM * BK + BK * BN)), stream>>>(
                transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, (float *)C, ldc);
}
...
tinyblasStatus_t tinyblasGemmEx(tinyblasHandle_t stream, //
    ...
    float alpha0, beta0;
    if (computeType == TINYBLAS_COMPUTE_32F) {
        alpha0 = *(float *)alpha;
        beta0 = *(float *)beta;
    } else /* computeType == TINYBLAS_COMPUTE_16F */ {
        alpha0 = __half2float(*(half *)alpha);
        beta0 = __half2float(*(half *)beta);
    }

    tinyblasGE_wrapper<48, 32, 64, 3, 8, half>(stream, transa, transb, m, n, k, alpha0,
...
static __global__ void tinyblasGBE_entry(tinyblasOperation_t transa, //
    ...
    for (z = blockIdx.z; z < batchCount; z += jump3)
        for (x = blockIdx.x * BM; x < m; x += jump1)
            for (y = blockIdx.y * BN; y < n; y += jump2)
                matmul_block2d<BM, BN, BK, TM, TN, SRC, DST>(transa, transb, m, n, k, x, y, alpha,
...
static void tinyblasGBE_wrapper(tinyblasHandle_t stream, //
    ...
    if (Ctype == CUDA_R_16F)
        tinyblasGBE_entry<BM, BN, BK, TM, TN, SRC, half>
            <<<maxblocks, maxthreads, (sizeof(SRC) * (BM * BK + BK * BN)), stream>>>(
                transa, transb, m, n, k, __float2half(alpha), Aarray, lda, Barray, ldb,
                __float2half(beta), Carray, ldc, batchCount);
    else
        tinyblasGBE_entry<BM, BN, BK, TM, TN, SRC, float>
            <<<maxblocks, maxthreads, (sizeof(SRC) * (BM * BK + BK * BN)), stream>>>(
...
tinyblasStatus_t tinyblasGemmBatchedEx(tinyblasHandle_t stream, //
    ...
    tinyblasGBE_wrapper<48, 32, 64, 3, 8, half>(stream, transa, transb, //
                                                m, n, k, //
                                                alpha0, //
                                                (const half **)Aarray, lda, //
                                                (const half **)Barray, ldb, //
                                                beta0, Carray, Ctype, ldc, //
                                                batchCount);
    return TINYBLAS_STATUS_SUCCESS;
}
...
template <int BM, int BN, int BK, int TM, int TN, typename SRC, typename DST>
static __global__ void tinyblasGSBE_entry(tinyblasOperation_t transa, //
                                          tinyblasOperation_t transb, //
                                          int m, int n, int k, //
                                          const DST alpha, //
    ...
    for (z = blockIdx.z; z < batchCount; z += jump3)
        for (x = blockIdx.x * BM; x < m; x += jump1)
            for (y = blockIdx.y * BN; y < n; y += jump2)
                matmul_block2d<BM, BN, BK, TM, TN, SRC, DST>(transa, transb, //
                                                             m, n, k, x, y, //
                                                             alpha, //
                                                             A + z * strideA, lda, //
                                                             B + z * strideB, ldb, //
...
tinyblasStatus_t tinyblasGemmStridedBatchedEx(tinyblasHandle_t stream, //
    ...
    if (computeType == TINYBLAS_COMPUTE_32F) {
        ...
        alpha0 = __half2float(*(half *)pAlpha);
        beta0 = __half2float(*(half *)pBeta);
    }

    tinyblasGSBE_wrapper<32, 4, 64, 1, 2, half>(stream, transa, transb, m, n, k, alpha0,
                                                (const half *)A, lda, strideA, (const half *)B, ldb,
                                                strideB, beta0, C, Ctype, ldc, strideC, batchCount);
