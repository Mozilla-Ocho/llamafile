...
static bool is_immediate(int c) {
    switch (c) {
    case '&':
    case '*':
    case '+':
...
void HighlightAsm::feed(std::string *r, std::string_view input) {
            ...
            if (!isascii(c) || isalnum(c) || c == '$' || c == '_' || c == '-' || c == '.') {
                ...
                if (is_first_thing_on_line_) {
                    if (word_.size() > 1 && word_[0] == '#' &&
                        ...
                        *r += word_;
                        *r += HI_RESET;
                    } else if (word_ == "C" || word_ == "dnl" || word_ == "m4_dnl") {
                        *r += HI_COMMENT;
                        *r += word_;
                        word_.clear();
                        t_ = COMMENT;
                ...
                } else if (is_preprocessor_ && is_keyword_c_builtin(word_.data(), word_.size())) {
                    *r += HI_BUILTIN;
                    *r += word_;
                    *r += HI_RESET;
                } else if (is_keyword_asm_qualifier(word_.data(), word_.size())) {
                    *r += HI_QUALIFIER;
                    *r += word_;
                    *r += HI_RESET;
...
void HighlightAsm::flush(std::string *r) {
    switch (t_) {
    case WORD:
        *r += word_;
        word_.clear();
